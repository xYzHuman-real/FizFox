...
app = FastAPI(title="FizFox API", version="0.1.0", description="Backend foundation for the FizFox AI application builder.")
...
preview_builder = StaticPreviewBuilder()
...
def _build_result(success: bool, diagnostics: list) -> BuildResult:
    return BuildResult(success=success, diagnostics=[RuntimeDiagnosticModel(level=d.level, code=d.code, message=d.message, file=d.file) for d in diagnostics])
...
def _run_build(project: Project) -> Project:
    ...
    if verification_result.success:
        project.preview_html = preview_builder.build(project.files)
    return project
...
def generate_project(project_id: str) -> Project:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if project.spec is None: raise HTTPException(status_code=409, detail="Project must be planned before generation")
...
def edit_project(project_id: str, request: EditProjectRequest) -> Project:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if not project.files: raise HTTPException(status_code=409, detail="Project must be generated before editing")
    project.status = ProjectStatus.EDITING
    project.files = editor.edit(project.files, request.instruction)
    project.build = None
    project.preview_html = None
    project.status = ProjectStatus.GENERATED
    return project
...
def build_project(project_id: str) -> Project:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if not project.files: raise HTTPException(status_code=409, detail="Project must be generated before building")
...
def verify_project(project_id: str) -> Project:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if not project.files: raise HTTPException(status_code=409, detail="Project must be generated before verification")
    project.status = ProjectStatus.VERIFYING
    ...
    if result.success: project.preview_html = preview_builder.build(project.files)
    return project
...
def repair_project(project_id: str) -> Project:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if project.spec is None or not project.files: raise HTTPException(status_code=409, detail="Project must be planned and generated before repair")
    ...
    project.build = _build_result(not any(d.level == "error" for d in result.diagnostics), result.diagnostics)
    project.status = ProjectStatus.READY if project.build.success else ProjectStatus.FAILED
    if project.build.success: project.preview_html = preview_builder.build(project.files)
    return project
...
def build_and_repair_project(project_id: str) -> Project:
    ...
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if not project.files: raise HTTPException(status_code=409, detail="Project must be generated before building")
    if project.spec is None: raise HTTPException(status_code=409, detail="Project must be planned before repair")
    project = _run_build(project)
...
def get_project_preview(project_id: str) -> dict[str, str]:
    project = projects.get(project_id)
    if project is None: raise HTTPException(status_code=404, detail="Project not found")
    if not project.preview_html: raise HTTPException(status_code=409, detail="Project must pass build and verification before preview")
