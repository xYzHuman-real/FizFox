...
- `FIZFOX_ALLOWED_ORIGINS` — comma-separated frontend origins, for example `https://xYzHuman-real.github.io`.
...
The browser stores that API address locally, so later visits can use the normal Pages URL.
...
The current worker performs constrained validation; a production live-preview service still needs an isolated preview runtime, controlled ingress, resource quotas, cleanup, and authentication/rate limiting.
